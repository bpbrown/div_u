...
taus = [tau_c1, tau_b1, tau_b2, tau_u1, tau_u2]
...
grad_u = d3.grad(u) + 1/nu*ez*lift(tau_u1) # First-order reduction
grad_b = d3.grad(b) + 1/kappa*ez*lift(tau_b1) # First-order reduction
...
problem.add_equation("trace(grad_u) + tau_div = 0")
problem.add_equation("dt(b) - ez@u - kappa*div(grad_b) + tau_b = - u@grad(b)")
problem.add_equation("dt(u) - nu*div(grad_u) + grad(p) - b*ez + tau_u = - u@grad(u)")
